...
class FactorGraph:
    ...
    def runParallelLoopyBP(self, iterations):
        ...
        for iteration in range(iterations):
            for variable_Y in range(len(self.var)):
                related_factors = self.varToFactor[variable_Y]
                for current_factor in related_factors:
                    unnormalized_massage =0
                    flag = 0;
                    for other_factor in related_factors:
                        if(other_factor != current_factor):
                            Message_otherF_to_Y = self.getInMessage(other_factor, variable_Y, type="factorToVar")
                            if(flag == 0 ):
                                unnormalized_massage = Message_otherF_to_Y
                                flag = 1
                            else:
                                unnormalized_massage = unnormalized_massage.multiply(Message_otherF_to_Y)
                    self.messagesVarToFactor[(variable_Y, current_factor)] = unnormalized_massage.normalize()
            
            
            
            for factor in range(len(self.factorToVar)):
                related_variables = self.factorToVar[factor]
                for current_variable in related_variables:
                    unnormalized_massage = self.factors[factor]
                    #print("asghar")
                    #print(unnormalized_massage)
                    flag = 0;
                    for other_variable in related_variables:
                        if(other_variable != current_variable):
                            #continue
                            Message_otherY_to_F = self.getInMessage(other_variable, factor, type="varToFactor")
                            unnormalized_massage = unnormalized_massage.multiply(Message_otherY_to_F)

                    unnormalized_massage = unnormalized_massage.marginalize_all_but([current_variable])
                    self.messagesFactorToVar[(factor, current_variable)] = unnormalized_massage.normalize()
